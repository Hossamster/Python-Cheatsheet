#set methods
#add
s = {1,2,3,4}
s.add(5)
print(s)
s.add("Hello")
print(s)

#remove
lol = {1,2,3,4,5,6}
lol.remove(3)
print(lol)
# lol.remove(888) # error
#to avoid error use discard()
lol.discard(7234) #no error 
print(lol)

#copy
k = lol.copy()
print(k)
k.clear()
print(k)

#math set
#union
math_std = {"Hossam","Ahmed","Manar","jana"}
bio_std = {"Ayat","Menna","Hossam","Abdo"}
u = math_std | bio_std # union
print(u)

#intersection
inte = math_std & bio_std
print(inte)

#diff
dif = math_std - bio_std
print(dif)



"""
# 1 - Create a variable called numbers which is a tuple with the the values 1, 2, 3 and 4 inside.
numbers = (1,2,3,4)

# 2 - Create a variable called value which is a tuple with the the value 1 inside.
value = (1,)

# 3 - Given the following variable:

values = [10,20,30]

# Create a variable called static_values which is the result of the values variable converted to a tuple
static_values = tuple(values)
print(static_values)
# 4 - Given the following variable

stuff = [1,3,1,5,2,5,1,2,5]

# Create a variable called unique_stuff which is a set of only the unique values in the stuff list
unique_stuff = set(stuff)
print(unique_stuff)

"""